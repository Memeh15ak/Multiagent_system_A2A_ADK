...
from Agents.report_gen_agent.report_adk import content_agent

logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)

class ADKReportAgentExecutor(AgentExecutor):
    """An AgentExecutor that runs an ADK-based Report Agent."""
    ...
    async def _get_agent_and_runner(self):
        ...
        if self._agent is None:
            # Fix: content_agent is already an LlmAgent object, not a function
            self._agent = content_agent  # Remove the () call and await
            self.runner = Runner(
    ...
    async def _process_request(
        ...
        async for event in self._run_agent(
            ...
            if event.is_final_response():
                ...
                # Update status to indicate processing is in progress
                task_updater.update_status(
                    TaskState.working,
                    message=task_updater.new_agent_message(
                        convert_genai_parts_to_a2a(event.content.parts)
