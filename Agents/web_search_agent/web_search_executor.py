...
class ADKWebSearchAgentExecutor(AgentExecutor):
    """An AgentExecutor that runs an ADK-based Web Search Agent."""
    ...
    async def _process_request(
        ...
        async for event in self._run_agent(
            ...
            if event.is_final_response():
                ...
                # Handle function calls if any (though Perplexity has built-in search)
                logger.debug('Processing event with function call: %s', event.get_function_calls())
                # Update status to indicate search is in progress
                task_updater.update_status(
                    TaskState.working,
                    message=task_updater.new_agent_message(
                        convert_genai_parts_to_a2a(event.content.parts)
