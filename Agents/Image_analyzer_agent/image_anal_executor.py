...
from Agents.Image_analyzer_agent.image_anal_adk import create_image_agent, image_manager 

logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)

class ADKImageAgentExecutor(AgentExecutor):
    """An AgentExecutor that runs the ADK-based Image Analysis Agent."""

    def __init__(self):
        self._agent = create_image_agent
        self.runner = Runner(
    ...
    async def execute(
        ...
        logger.info(f"Cleaning up previous ImageManager state for task {context.task_id}...")
        image_manager.cleanup()
        logger.info("ImageManager state cleaned.")
    ...
    async def _process_request(
        ...
        async for event in self.runner.run_async(session_id=session_id, user_id='a2a_user', new_message=new_message):
            if event.is_final_response():
                ...
                task_updater.add_artifact(response_parts or [TextPart(text="Image analysis completed.")])
        ...
        if not has_final_response:
            ...
            task_updater.failed(error_message="Task did not complete successfully.")
